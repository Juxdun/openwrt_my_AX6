...
#-------------------------------------------------------------------------------------------------------------------------------
# Modify the version number版本号里显示一个自己的名字（Juxdun $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i 's/OpenWrt /Juxdun ALPHA $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g' package/lean/default-settings/files/zzz-default-settings
...
#-------------------------------------------------------------------------------------------------------------------------------
# 修改默认wifi名称ssid为Redmi_AX6
#sed -i 's/ssid=OpenWrt/ssid=Redmi_AX6/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh

# 修改默认wifi密码key为23333333
sed -i 's/encryption=none/encryption=sae-mixed/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh
#使用sed 在第四行后添加新字
sed -i '/set wireless.default_radio${devidx}.encryption=sae-mixed/a\set wireless.default_radio${devidx}.key=23333333' package/kernel/mac80211/files/lib/wifi/mac80211.sh
...
#5g
sed -i '/ set wireless.default_radio0${devidx}.ssid=OpenWrt/a\set wireless.default_radio0${devidx}.ssid=Redmi_AX6_5G' package/kernel/mac80211/files/lib/wifi/mac80211.sh
#2.4g
sed -i '/ set wireless.default_radio1${devidx}.ssid=OpenWrt/a\set wireless.default_radio1${devidx}.ssid=Redmi_AX6' package/kernel/mac80211/files/lib/wifi/mac80211.sh
